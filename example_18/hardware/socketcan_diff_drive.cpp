  ...
  const hardware_interface::HardwareComponentInterfaceParams & params)
{
  ...
  if (info_.joints.size() != 2 || info_.joints[0].command_interfaces.size() != 1 ||
  ...
  if (!transport_) {return hardware_interface::CallbackReturn::ERROR;}
...
hardware_interface::CallbackReturn SocketCanDiffDriveHardware::on_activate(
  const rclcpp_lifecycle::State &) {return hardware_interface::CallbackReturn::SUCCESS;}
  ...
  const rclcpp::Duration &)
{
  ...
  if (!transport_ || !std::isfinite(command.left_velocity) ||
...
PLUGINLIB_EXPORT_CLASS(ros2_control_demo_example_18::SocketCanDiffDriveHardware,
